...
int main(int argc, char *argv[]){
   ...
   //should be handled at server side
   ...
   if(type_err < 0){
      perror("Writing to server socket\n");
      exit(1);
   }
   ...
   if(client_id != server_id){
      perror("Error: connecting to the wrong server");
      shutdown(sockFD,2);
      close(sockFD);
       exit(1);
   }
